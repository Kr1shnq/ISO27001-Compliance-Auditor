<#
.SYNOPSIS
    Collects Windows system configuration telemetry for the ISO 27001 Compliance Auditor.

.DESCRIPTION
    Performs a read-only inventory of the local Windows endpoint or server and emits a
    configuration report in the schema expected by the ISO 27001 Compliance Auditor web
    application. Covers identity and password policy, encryption, logging and audit policy,
    endpoint protection, patching, network exposure, backup, asset inventory, services,
    capacity and hardware health.

    The script is strictly READ-ONLY. It changes no setting, installs nothing and opens no
    outbound connection. Every probe is wrapped so that a failure on one item degrades that
    single value to $null ("not collected") rather than aborting the run.

.PARAMETER OutputPath
    Directory to write the reports to. Defaults to the current directory.

.PARAMETER Format
    Json, Csv or Both (default). Json is the richest; Csv is a flat key/value file for
    systems that can only move tabular data.

.PARAMETER IncludeAttestations
    Emit an empty 'attestations' block so an auditor can pre-fill procedural control answers
    before uploading. Ignored for CSV output.

.PARAMETER ApprovedSoftware
    Optional path to a text file listing approved application display names (one per line).
    Installed software not on the list is reported under asset.unauthorized_software.
...
.EXAMPLE
    ...
    Collects telemetry and writes JSON + CSV to the current folder.

.EXAMPLE
    .\Collect-ISOTelemetry.ps1 -OutputPath C:\Audit -Format Json -IncludeAttestations
    Writes a JSON report with an empty attestation block to C:\Audit.

.EXAMPLE
    Get-Content .\hosts.txt | ForEach-Object {
        Invoke-Command -ComputerName $_ -FilePath .\Collect-ISOTelemetry.ps1
    }
    Fan the collector out across a fleet (requires WinRM).

.NOTES
    Author  : ISO 27001 Compliance Auditor
    Version : 1.0.0
    Requires: Windows PowerShell 5.1 or PowerShell 7+, run as Administrator.
              Without elevation the BitLocker, audit policy, Defender and firewall probes
              return $null and the affected controls report as NO_DATA.
#>
